...
Chestnut is a programming language compiler and virtual machine (VM) written in C. It compiles `.cn` source files into bytecode and executes them on its own VM.
...
| Multiple source files | Compile multiple `.cn` files together in one invocation |
...
The CLI currently compiles source files and immediately executes them on the VM. It **overwrites `test.cb` in the working directory** with the generated bytecode, while execution loads the IR generated in memory. There are no separate compile-only or `.cb` execution options yet. Debug information may appear alongside program output.

## Syntax Example

Save the following as `hello.cn` and run it with `./chestnut hello.cn` or `.\chestnut.exe hello.cn`:
...
| [`test.cn`](test.cn), [`test2.cn`](test2.cn) | Multiple source files, classes, inheritance, constructors, and arrays |
...
.cn source files
  ...
  → VM metadata and instruction loading
  → main execution
```
