...
struct IRReader *gen_ir_reader(struct IRContext *irc) {
    ...
    reader->classes = NULL;
    reader->class_count = 0;
    reader->class_capacity = 0;
    reader->string_base = 0;
    return reader;
}
...
static bool read_u32(struct IRReader *reader, uint32_t *value) {
    ...
    if (!has_bytes(reader, 4))
        return false;
    for (i = 0; i < 4; i++)
        result |= (uint32_t)reader->bytes[reader->reader_cnt++] << (i * 8);
    *value = result;
    return true;
}
...
static unsigned relocated_class_id(const struct IRReader *reader, unsigned local_id) {
    unsigned i;
    for (i = 0; i < reader->class_count; i++)
        if (reader->classes[i].local_id == local_id)
            return reader->classes[i].runtime_id;
    return 0;
}

static unsigned class_runtime_id(const struct VM *vm, unsigned local_id, const char *name) {
    unsigned i;
    unsigned next_id = 1;

    for (i = 1; i < vm->class_data_capacity; i++) {
        const struct VMClassData *data = vm->class_data[i];
        if (data == NULL)
            continue;
    ...
    return vm_find_class_data(vm, local_id) == NULL ? local_id : next_id;
}

static bool register_class_relocation(struct IRReader *reader, unsigned local_id,
                                      unsigned runtime_id, unsigned parent_local_id) {
    unsigned i;
    struct ClassRelocation *entry;

    for (i = 0; i < reader->class_count; i++)
        if (reader->classes[i].local_id == local_id ||
            (runtime_id != 0 && reader->classes[i].runtime_id == runtime_id))
            return reader_error(reader, "duplicate class metadata");
    if (runtime_id > INT32_MAX)
        return reader_error(reader, "class id exceeds instruction range");
    if (reader->class_count == reader->class_capacity) {
        reader->class_capacity = reader->class_capacity == 0 ? 8 : reader->class_capacity * 2;
        reader->classes = S_realloc(reader->classes, sizeof(*reader->classes) * reader->class_capacity);
    }
    entry = &reader->classes[reader->class_count++];
    entry->local_id = local_id;
    entry->runtime_id = runtime_id;
    entry->parent_local_id = parent_local_id;
    return true;
}

static bool relocate_parent_classes(struct VM *vm, struct IRReader *reader) {
    unsigned i;
    for (i = 0; i < reader->class_count; i++) {
        const struct ClassRelocation *entry = &reader->classes[i];
        unsigned parent_id = relocated_class_id(reader, entry->parent_local_id);
        if (entry->parent_local_id != 0 && parent_id == 0)
            return reader_error(reader, "unresolved parent class id");
        vm_find_class_data(vm, entry->runtime_id)->parent_id = parent_id;
    }
    return true;
}
...
static bool decode_function_instructions(const byte *code, unsigned code_size,
    ...
    while (has_bytes(&code_reader, 1)) {
        ...
        if (is_operandless_instruction(opcode)) {
            ...
            operand_count = 1;
            if (!read_byte(&code_reader, &instruction->operands.u8)) {
                error_message = "truncated expression operand";
                goto fail;
            }
            ...
            memcpy(&instruction->operands.f32, &raw, sizeof(raw));
            ...
            memcpy(&instruction->operands.f64, &raw, sizeof(raw));
        } else {
            bool ok = true;
            ...
            for (i = 0; i < (unsigned)operand_count; i++)
                instruction->operands.i32[i] = read_i32(&code_reader, &ok);
            if (!ok) {
                error_message = "truncated instruction operand";
                goto fail;
            }
        }

        if (opcode == OP_NEW_OBJECT || opcode == OP_CALL_SUPER) {
            int32_t local_id = instruction->operands.i32[0];
            unsigned runtime_id = local_id > 0 ? relocated_class_id(source_reader, (unsigned)local_id) : 0;
            if (runtime_id == 0) {
                error_message = "unresolved instruction class id";
                goto fail;
            }
            instruction->operands.i32[0] = (int32_t)runtime_id;
        }

        if (opcode == OP_LOAD_STR) {
            int32_t id = instruction->operands.i32[0];
            if (id < 0 || source_reader->string_base > (unsigned)INT32_MAX - (unsigned)id) {
                error_message = "invalid string reference";
                goto fail;
            }
            instruction->operands.i32[0] = id + (int32_t)source_reader->string_base;
        }
    ...
    for (i = 0; i < instruction_count; i++) {
        ...
        if (instruction->opcode == OP_GOTO || instruction->opcode == OP_JE ||
            ...
            instruction->operands.u32[0] = label_targets[label_id];
        }
    }

    if (instruction_count != 0 && instruction_count < code_size)
        instructions = (struct VMInstruction *)S_realloc(
            instructions, sizeof(struct VMInstruction) * instruction_count);
...
static bool read_function_metadata(struct VM *vm, struct IRReader *reader,
    ...
    if (!ok || id < 0 || argument_count < 0 ||
        (unsigned)argument_count > reader->byte_cnt - reader->reader_cnt)
    ...
    for (i = 0; i < (unsigned)argument_count; i++) {
        ...
        if (!ok) {
            free(argument_types);
            return reader_error(reader, "truncated function arguments");
        }
    }
...
static bool read_meta_block(struct VM *vm, struct IRReader *reader, struct VMClassData *owner) {
    ...
    case META_CLASS: {
        ...
        unsigned runtime_id = class_runtime_id(vm, (unsigned)id, name);
...
static bool read_function(struct VM *vm, struct IRReader *reader, struct VMClassData *owner) {
    ...
    if (function_data == NULL)
        return reader_error(reader, "function code has no matching metadata");
...
static bool read_class(struct VM *vm, struct IRReader *reader) {
    ...
    class_data = vm_find_class_data(vm, relocated_class_id(reader, (unsigned)id));
...
static bool read_rodata(struct VM *vm, struct IRReader *reader) {
    ...
    while (peek_byte(reader, &kind) && kind != RODATA_END) {
        ...
        if (!ok || id < 0 || reader->string_base > (unsigned)INT32_MAX - (unsigned)id)
            return reader_error(reader, "invalid string rodata");

        register_string_pool(vm, (char *)value, id + (int)reader->string_base);
    }
...
static bool read_program_sections(struct VM *vm, struct IRReader *reader) {
    byte section;
    reader->string_base = vm->vm_string_pool->size;
    if (!read_byte(reader, &section) || section != CODE_BEGIN || !read_code(vm, reader))
        return reader_error(reader, "invalid code section");
    if (!read_byte(reader, &section) || section != RODATA_BEGIN || !read_rodata(vm, reader))
        return reader_error(reader, "invalid rodata section");
    if (has_bytes(reader, 1))
        return reader_error(reader, "unexpected data after rodata");
    return true;
}
...
bool read_ir_files(struct VM *vm, struct IRReader **readers, unsigned count) {
    unsigned i;
    for (i = 0; i < count; i++) {
        struct IRReader *reader = readers[i];
        byte section;
        assert(reader != NULL && reader->irc != NULL);
        reader->class_count = 0;
        if (!read_byte(reader, &section) || section != META_BEGIN || !read_meta(vm, reader))
            return reader_error(reader, "invalid metadata section");
    }
    for (i = 0; i < count; i++)
        if (!relocate_parent_classes(vm, readers[i]))
            return false;
    for (i = 0; i < count; i++)
        if (!read_program_sections(vm, readers[i]))
            return false;
...
bool read_ir(struct VM *vm, struct IRReader *reader) {
    return read_ir_files(vm, &reader, 1);
}
