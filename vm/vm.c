...
static const char *get_instruction_name(unsigned char opcode) {
	...
	case OP_EXPR_OP: return "expr_op";
...
static void debug_print_instruction(
	...
	if (instruction->opcode == OP_EXPR_OP) {
		const char *expr_name =
			get_expr_op_name(instruction->operands.u8);
...
bool exec_instruction(struct VM *vm,
	...
	default: {
		DEBUG_FPRINTF(stderr, "VM execution error at instruction %u: "
...
void vm_exec_function(struct VM *vm,
	...
	while (instruction_index < function_data->instruction_count) {
		...
		const char *command_name =
			get_instruction_name(instruction->opcode);
