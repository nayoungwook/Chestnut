...
static void patch_int(struct IRContext *irc, unsigned offset, int si) {
    ...
    assert(offset <= irc->byte_cnt && sizeof(int) <= irc->byte_cnt - offset);
    for (i = 0; i < sizeof(int); i++) {
...
static void gen_node_ir(struct IRContext *irc, struct ParserContext *pc, struct Node *node) {
    ...
    case AST_VariableDeclaration: {
        ...
        if (var_decl_ast->var_data != NULL) {
            ...
            case ScopeLocal:
                ...
                break;
            case ScopeGlobal:
                emit_byte(irc, OP_SAVE_GLOBAL);
                break;
            case ScopeClass:
                emit_byte(irc, OP_SAVE_CLASS);
                break;
            default:
    ...
    case AST_NumberLiteral: {
        ...
        if (numeric_data->is_integer) {
            ...
            if (strcmp(numeric_type->type_str, "double") == 0) {
                ...
                emit_double(irc, atof(num_lit_ast->num_tok->str));
            }
        }
    ...
    case AST_FunctionCall: {
        ...
        if (!func_data->varargs) {
            assert(func_data->arg_count == func_call_ast->param_count);
        }
